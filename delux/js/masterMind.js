...
{
	var ctrl = this;
	ctrl.getData=function (Query,TableName,filter)
	{
			var sqlQuery=Query;
			if(filter!=undefined )
			{
				if(filter.length>0)
				sqlQuery=Query+' Where '+filter;
			}
			$http.get(URL+'?Query='+sqlQuery ).then(function (response) 
			{
           $scope[TableName]= response.data.listdata;
			});
		ctrl.SelectCombox=function(TableName,fieldvalue,SelectField,onchange,DefaultData)
	   {
		    if(SelectField==undefined)
			   return;
		   else
		   {
				var formula=SelectField.split("|");
...
function dynamicDetailsController($scope, $element, $attrs,$http) 
{
    ...
    ctrl.AddRecord = function () 
	{
			...
			ctrl.editDialog.open(ctrl["Temp"+ctrl.tablename] , ctrl.formmodelfields,"Add Item","Add");
    } 
	ctrl.editrecord1=function(object,index)
	{
              $scope.ModelButton = "Edit";
              ctrl.TempTrandetails = {};
			  ctrl["Temp"+ctrl.tablename] = {};
              ctrl["Temp"+ctrl.tablename]= angular.copy(object);
		      ctrl.editDialog.open(  ctrl["Temp"+ctrl.tablename], ctrl.formmodelfields,"Edit Item","Edit");
	}
	ctrl.deleterecord1=function(object,index)
	{
    ...
    ctrl.pushrecord = function (object) 
	{
		...
		angular.forEach(ctrl.formmodelfields,function(object)
		{
	...
	ctrl.TotalAccount=function(datalist,key)
	{
				var total = 0;
...
EditPersonDialogModel.prototype.open = function(object, formmodelfields,title,ActionName) 
{
	this.title=	title;
	this.object = object;
