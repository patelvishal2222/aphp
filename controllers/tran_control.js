...
myApp.controller('tran_control', function ($scope, $state, $http, $location,$stateParams,$filter,URL) {
    var vm = this;
	...
	vm.dbdata={};
	 ...
	 //ItemNames
        $http.get(URL+'?Query=select *  from ItemMaster' ).then(function (response) {
           $scope.ItemNames = response.data.listdata;
			
       });
	   
	   //AccountMasters
	   $http.get(URL+'?Query=select *  from AccountMaster' ).then(function (response) {
           $scope.AccountMasters = response.data.listdata;
			
       });
	   //CrDrMasters
	   $http.get(URL+'?Query=select *  from CrDrMaster' ).then(function (response) {
	 ...
	 //AddRecord
        ...
        $http.post(URL, tempAccountMaster).then(function (response) {
	...
	else
	{
        ...
        $http.get(encodeURI(URL+"?List=1&Sql=SELECT * FROM vwTranscation &TableName=vwTranscation&Page="+page_number+"&PageSize=10&OrderBy=TranId   desc&Search=AccountName like '%"+search_input+"%'")).then(function (response) {
