myApp.controller('accountdetailsControl', function ($scope, $state, $http, $location,$stateParams) {
	  ...
	  //AccountMaster
	   ...
	   //AccountDetails
	    $http.get(encodeURI('php/DyamicOperation.php?Query=select *,@balance:=@balance@plus@bal  as Balance from vwaccountdetails join (Select @balance:=0) x where AccountMasterId='+ vm.AccountMasterId.toString())).then(function (response) {
